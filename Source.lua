-- DEX v3.2.0 [modified by wifi]
-- Using https://lua.expert API
-- Developed by ZxL
...
local function main()
	...
	ScriptViewer.Init = function()
	    ...
	    save.MouseButton1Click:Connect(function()
			...
			if PreviousScr then
				local cleanName = env.parsefile(PreviousScr.Name)
				filename = "dex/saved/" .. cleanName .. "_Decompiled.lua"
			else
				filename = "dex/saved/Scriptviewer_" .. os.date("%Y%m%d_%H%M%S") .. ".lua"
			end
...
Main = (function()
	...
	env.decompile = function(script_instance)
		...
		if mode == "Default" then
			...
			if not success or not bytecode then
				return "-- Failed to get script bytecode. Your executor does not support getscriptbytecode."
			end

			local elapsed = os.clock() - last_lua_expert_call
			if elapsed < 0.6 then
			...
			local req = env.request
			if not req then
				return "-- HTTP Request not supported by your executor"
			end
		...
		elseif mode == "Shiny" then
			local success, bytecode = pcall(env.getscriptbytecode, script_instance)
			if not success or not bytecode then return "-- getscriptbytecode missing" end
			local base64encode = (crypt and crypt.base64encode) or (base64 and base64.encode)
			if not base64encode then return "-- Base64 encode missing" end
			local encoded = base64encode(bytecode)
			local req = env.request
			if not req then return "-- HTTP Request not supported" end
			local successReq, res = pcall(req, {
				Url = "http://127.0.0.1:" .. tostring(Settings.ShinyDecompilerPort) .. "/luau/decompile",
				Method = "POST", Body = encoded
			})
			if successReq and res and res.Body then return res.Body else return "-- request failed" end
	...
	Main.CreateIntro = function(initStatus)
		local gui = create({
			...
			{11,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=3,Name="Creator",Parent={2},Position=UDim2.new(1,-130,1,-20),Size=UDim2.new(0,125,0,20),Text="modified by wifi",TextColor3=Settings.Theme.Text,TextSize=14,TextXAlignment=1,}},
